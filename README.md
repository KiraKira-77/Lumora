...
- 支持把桌面应用图标拖入 Launcher 的某个快捷键槽，点击槽位或按对应键位启动。
- 支持 Windows Shell 图标提取，提取成功后 Dock 和快捷键槽显示真实应用图标。
- 支持基础文件搜索和桌面文件分类整理能力。
...
- 真实应用图标提取当前使用 `SHGetFileInfoW`，能覆盖常见 `.lnk/.exe` 场景；UWP/Store 应用和更高清图标后续需要接 `IShellItemImageFactory`。
